...
if($type=='alipay'&&$conf['alipay_api']==5 || $type=='tenpay'&&$conf['qqpay_api']==5 || $type=='qqpay'&&$conf['qqpay_api']==5 || $type=='wxpay'&&$conf['wxpay_api']==5){
	echo "<script>window.location.href='./codepay.php?type={$type}&trade_no={$orderid}';</script>";
	exit;
}elseif($type=='alipay'&&$conf['alipay_api']==6 || $type=='tenpay'&&$conf['qqpay_api']==6 || $type=='qqpay'&&$conf['qqpay_api']==6 || $type=='wxpay'&&$conf['wxpay_api']==6){
	require_once(SYSTEM_ROOT."epay/blpay.config.php");
	require_once(SYSTEM_ROOT."epay/epay_submit.class.php");
	$parameter = array(
		"pid" => trim($alipay_config['partner']),
		"type" => $type,
		"notify_url"	=> $siteurl.'blpay_notify.php',
		"return_url"	=> $siteurl.'blpay_return.php',
...
}elseif($type=='alipay'&&$conf['alipay_api']==2 || $type=='tenpay'&&$conf['tenpay_api']==2 || $type=='qqpay'&&$conf['qqpay_api']==2 || $type=='wxpay'&&$conf['wxpay_api']==2){
