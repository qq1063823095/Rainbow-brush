...
function getshop(tid,cid,name,price,input,inputs,multi,isfaka,value,alert,shopimg){
	$('#display_selectclass').hide();
	$('#shoplist').hide(); 
	$('#shopinfo').show();
	$("#selected").html('填写商品信息 <select name="tid" id="tid"><option value="'+tid+'" cid="'+cid+'" price="'+price+'" inputname="'+input+'" inputsname="'+inputs+'" multi="'+multi+'" isfaka="'+isfaka+'" count="'+value+'" alert="'+alert+'" shopimg="'+shopimg+'">'+name+'</option></select><a class="btn btn-success btn-xs pull-right" href="./?cid='+cid+'">返回重选</a>');
	$("#infoshop").html('<div class="form-group text-center"><img src="'+shopimg+'" width="120" height="120" style="border-radius: 8px" id="shoptypes" onerror="this.src=\'assets/img/Product/noimg.png\'"><hr class="layui-bg-blue"><b>当前商品：'+generate(name)+(alert!=''?'<br><button class="btn btn-default btn-sm" onclick="showAlert()">下单须知</button>':'')+'<hr class="layui-bg-orange">商品价格：<font color="red">￥'+price+'</font></b>');
...
function getkuaishouid(){
	...
	if(kuauishouurl.indexOf('gifshow.com')<0 && kuauishouurl.indexOf('kuaishou.com')<0 && kuauishouurl.indexOf('kwai.com')<0 && kuauishouurl.indexOf('kspkg.com')<0){layer.alert('请输入正确的快手作品链接！');return false;}
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				$.each(data.data, function (i, res) {
					$("#shoplist").append('<div class="col-xs-6 col-sm-3 col-md-3 layui-anim layui-anim-scaleSpring" data-anim="layui-anim-upbit"><a href="javascript:void(0)" id="'+res.tid+'" onclick="getshop('+res.tid+',\''+cid+'\',\''+res.name+'\',\''+res.price+'\',\''+res.input+'\',\''+res.inputs+'\',\''+res.multi+'\',\''+res.isfaka+'\',\''+res.value+'\',\''+escape(res.alert)+'\',\''+res.shopimg+'\');" value="'+res.tid+'"><div class="thumbnail" style="height:240px;"><center style="margin-top:5%;"><img src="'+res.shopimg+'" width="80" height="100" style="border-radius: 15px" onerror="this.src=\'assets/img/Product/noimg.png\'"><hr class="layui-bg-blue" style="width:100%">'+res.name+'<hr class="layui-bg-red" style="width:100%">[￥'+res.price+']<br><span class="layui-badge layui-bg-blue">正常下单</span></center></div></a></div>');
					num++;
				});
				$("#shoplist").val(0);
				/*getPoint();*/
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				$.each(data.data, function (i, res) {
					...
					$("#shoplist").append('<div class="col-xs-6 col-sm-3 col-md-3 layui-anim layui-anim-scaleSpring" data-anim="layui-anim-upbit"><a href="javascript:void(0)" id="tools'+res.tid+'" onclick="getshop('+res.tid+',\''+cid+'\',\''+res.name+'\',\''+res.price+'\',\''+res.input+'\',\''+res.inputs+'\',\''+res.multi+'\',\''+res.isfaka+'\',\''+res.value+'\',\''+escape(res.alert)+'\',\''+res.shopimg+'\');"><div class="thumbnail" style="height:240px;"><center style="margin-top:5%;"><img src="'+res.shopimg+'" width="80" height="100" style="border-radius: 15px" onerror="this.src=\'assets/img/Product/noimg.png\'"><hr class="layui-bg-blue" style="width:100%">'+generate(res.name)+'<hr class="layui-bg-red" style="width:100%">[￥'+price+']<br><span class="layui-badge layui-bg-blue">立即购买</span></center></div></a></div>');
					num++;
				});
				if($_GET["tid"]){
